import { CodeLensProvider, Range, TextEditorDecorationType, window, TextDocument, CancellationToken, CodeLens, commands, SymbolInformation, SymbolKind, Location, ExtensionContext, Uri } from "vscode";
import { AppConfiguration } from "./AppConfiguration";
import { MethodReferenceLens } from "./MethodReferenceLens";
import Project, { ClassDeclaration, ClassMemberTypes, InterfaceDeclaration, ExpressionWithTypeArguments, PropertySignature, MethodSignature, PropertyDeclaration, MethodDeclaration } from "ts-simple-ast";
...
export class TSCodeLensProvider implements CodeLensProvider {
    ...
    getImplemetsMembers(locations: Location[], filters?: ExpressionWithTypeArguments[]): Array<PropertySignature | MethodSignature> {
      ...
      if (codeLens instanceof MethodReferenceLens) {
        const file = this.config.project.getSourceFile(
          ...
          const res = await Promise.all([commands.executeCommand<Location[]>('vscode.executeReferenceProvider', codeLens.uri, codeLens.range.start),
            commands.executeCommand<SymbolInformation[]>('vscode.executeDocumentSymbolProvider', window.activeTextEditor.document.uri)]);
            ...
            const testM = window.activeTextEditor.document.getText(
              codeLens.range
            );
